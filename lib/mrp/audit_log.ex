defmodule Store.AuditLog do
  ...
  A new audit action can be created by updating the `@params` module
  attribute with the action name and a list of allowed parameters that
  the log can store.

      @params %{
        ...,
        "my_new_action" => ~w(foo bar bazz)
      }

  The function initiating the audit now must recieve an AuditLog struct
  from the caller (information on how and where this struct is set can
  be found in `Web.RequestContext`). This struct will contain client
  indentifying info including:
  ...
  * User Email
  ...
  An AuditLog for testing purposes can be generated with `system/0`.
  This generates an AuditLog struct that can then be passed into
  the function requiring an audit_context.

      iex> Core.Accounts.register_user(Core.AuditLog.system(), %{...} = attrs)
      {:ok, %User{}}
  ...
  schema "audit_logs" do
    field(:action, :string)
    field(:ip_address, Extensions.Ecto.IPAddress)
  ...
  @params %{
    ...
    "accounts.reset_password.done" => ~w(user_id),
    "accounts.update_email.init" => ~w(user_id email),
    "accounts.update_email.done" => ~w(user_id email),
  ...
  @doc """
  Lists audits not attached to anything.
  """
